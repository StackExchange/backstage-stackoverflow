...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    ...
    await request(server).get('/api/stack-overflow-teams/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/stack-overflow-teams/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/stack-overflow-teams/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/stack-overflow-teams/todos/${createdTodoItem.id}`)
